...
var tinynlp = (function(){
    ...
    Chart.prototype.getCompleteState = function(){
        var lastColumn = this.chart[this.chart.length - 1];
        var candidate_return = [];
        for(var i in lastColumn){
            var state = lastColumn[i];
            if(state.complete()) {
                candidate_return.push(state);
            }
        }
        if(candidate_return != null){
            return candidate_return;
        }
        return null;
    }

    Chart.prototype.getPredictionState = function(){
        var lastColumn = this.chart[this.chart.length - 1];
        var candidate_return = [];
        for(var i in lastColumn){
            var state = lastColumn[i];
    ...
    State.prototype.prediction = function(){
        return this.left==0 && this.dot < this.rhs.length;
    }
    ...
    function parse(tokens, grammar, rootRule) {
        ...
        console.log("rootRuleRhss: " + rootRuleRhss);
        ...
        console.log("chartの初期化")
        //chart.log(0);
        console.log("アルゴリズム本体\n")
        for (var i = 0; i < tokens.length + 1; i++) {
